RSpec.describe CommentsController, type: :controller do
  ...
  describe "POST #create" do
    it "creates a new comment" do
      expect {
        post :create, params: { pan_id: pan.id, comment: { content: "Great pan!" } }, format: :json
    ...
    it "returns the comment in the response" do
      post :create, params: { pan_id: pan.id, comment: { content: "Great pan!" } }, format: :json
      expect(JSON.parse(response.body)['comment']['content']).to eq("Great pan!")
    end

    it "returns an error for invalid parameters" do
      post :create, params: { pan_id: pan.id, comment: { content: "" } }, format: :json
